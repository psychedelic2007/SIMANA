...
def calculate_sasa(uploaded_files, custom_labels):
    all_x_data = []
    all_y_data = []
    labels = []
...
def plot_sasa(all_x_data, all_y_data, labels, customizations):
    ...
    ax.set_title(f'Solvent Accessible Surface Area')
    ax.legend()
...
def plot_distribution(all_y_data, labels, customizations):
    ...
    ax.set_xlabel(customizations['x_label'], fontsize=customizations['x_label_size'])
    ax.set_ylabel(customizations['y_label'], fontsize=customizations['y_label_size'])
    ax.set_title('Solvent Accessible Surface Area Distribution')
    ax.legend()
...
def sasa():
    st.title("Solvent Accessible Surface Area Analysis")
    ...
    if st.button('Submit') and uploaded_files:
        all_x_data, all_y_data, labels = calculate_sasa(uploaded_files, custom_labels)
    ...
    st.sidebar.header("SASA Curve Plot Customization")
    ...
    st.sidebar.header("SASA Distribution Plot Customization")
    ...
    if st.session_state.data:
        ...
        fig_sasa = plot_sasa(all_x_data, all_y_data, labels, st.session_state.customizations_curve)
        st.pyplot(fig_sasa)
        
        download_link_sasa = get_download_link(fig_sasa, "sasa_curve")
        if download_link_sasa:
            st.markdown(download_link_sasa, unsafe_allow_html=True)
        
        fig_dist = plot_distribution(all_y_data, labels, st.session_state.customizations_dist)
        st.pyplot(fig_dist)
        
        download_link_dist = get_download_link(fig_dist, "sasa_distribution")
