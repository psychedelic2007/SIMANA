...
def calculate_rmsd(uploaded_files, custom_labels):
    all_x_data = []
    all_y_data = []
    labels = []
...
def plot_rmsd(all_x_data, all_y_data, labels, customizations):
    ...
    ax.set_title(f'Root Mean Square Deviation')
    ax.legend()
...
def plot_distribution(all_y_data, labels, customizations):
    ...
    ax.set_xlabel(customizations['x_label'], fontsize=customizations['x_label_size'])
    ax.set_ylabel(customizations['y_label'], fontsize=customizations['y_label_size'])
    ax.set_title('Root Mean Square Deviation Distribution')
    ax.legend()
...
def rmsd():
    st.title("Root Mean Square Deviation Analysis")
    ...
    if st.button('Submit') and uploaded_files:
        all_x_data, all_y_data, labels = calculate_rmsd(uploaded_files, custom_labels)
    ...
    if st.session_state.data:
        ...
        fig_rmsd = plot_rmsd(all_x_data, all_y_data, labels, st.session_state.customizations_curve)
        st.pyplot(fig_rmsd)
        
        download_link_rmsd = get_download_link(fig_rmsd, "rmsd_curve")
        if download_link_rmsd:
            st.markdown(download_link_rmsd, unsafe_allow_html=True)
        
        fig_dist = plot_distribution(all_y_data, labels, st.session_state.customizations_dist)
        st.pyplot(fig_dist)
        
        download_link_dist = get_download_link(fig_dist, "rmsd_distribution")
