...
def calculate_rg(uploaded_files, custom_labels):
    all_x_data = []
    all_y_data = []
    labels = []
...
def plot_rg(all_x_data, all_y_data, labels, customizations):
    ...
    ax.set_title(f'Radius of Gyration')
    ax.legend()
...
def rg():
    st.title("Radius of Gyration Analysis")
    ...
    if st.button('Submit') and uploaded_files:
        all_x_data, all_y_data, labels = calculate_rg(uploaded_files, custom_labels)
    ...
    st.sidebar.header("RoG Curve Plot Customization")
    ...
    st.sidebar.header("RoG Distribution Plot Customization")
    ...
    if st.session_state.data:
        ...
        fig_rg = plot_rg(all_x_data, all_y_data, labels, st.session_state.customizations_curve)
        st.pyplot(fig_rg)
        
        download_link_rg = get_download_link(fig_rg, "rg_curve")
        if download_link_rg:
            st.markdown(download_link_rg, unsafe_allow_html=True)
        
        fig_dist = plot_distribution(all_y_data, labels, st.session_state.customizations_dist)
        st.pyplot(fig_dist)
        
        download_link_dist = get_download_link(fig_dist, "rg_distribution")
